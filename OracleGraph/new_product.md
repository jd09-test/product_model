...
## 1. PGQL Node Property Safety

- Whenever writing PGQL, only use property names you have explicitly validated on that node/relationship type.
- Never invent property names. Always confirm keys using schema/tools if unsure.

### Relationship Join Field Enforcement

- For every relationship (as defined in schema or graph metadata, e.g. `AttributeVod_HAS_VERSION_VodVersion`), whenever a join (MATCH/WHERE/ON) is needed in PGQL, always use the property names given in the relationship's definition:
  - Use `source_vertex_column` as the property on the source node
  - Use `target_vertex_column` as the property on the target node
- Do NOT infer or guess join keys – ALWAYS extract and apply the correct `source_vertex_column`/`target_vertex_column` from the relationship definition.
- **Example:**
  - If `source_vertex_column: "ROW_ID"` and `target_vertex_column: "VERSIONED_OBJECT_DEFINITION_ID"`, your MATCH/WHERE must join these exact properties:
    ```
...
- Whenever a node property gets its value from another node’s property and the connection is established via a REFERS_TO (or equivalent) relationship, always use the explicit source_vertex_column and target_vertex_column as declared in the relationship type for PGQL joins.
- Never connect properties by guessing or by using similarly-named fields—enforce only schema-defined join paths.
- For example: If ObjectRelationship.SUB_OBJECT_PRODUCT_ID is connected by ObjectRelationship_REFERS_TO_ProductVod (source_vertex_column: SUB_OBJECT_PRODUCT_ID, target_vertex_column: OBJECT_NUMBER), always join SUB_OBJECT_PRODUCT_ID to the OBJECT_NUMBER of ProductVod, not to ROW_ID or any other field.
